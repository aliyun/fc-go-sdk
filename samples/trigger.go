...
func main() {
	...
	createTriggerInput := fc.NewCreateTriggerInput(serviceName, functionName).
		...
		WithInvocationRole("acs:ram:cn-hangzhou:123:role1").
		WithTriggerType("oss").
		WithSourceARN("acs:oss:cn-hangzhou:123:fcbucket").
		WithTriggerConfig(
			fc.NewOSSTriggerConfig().
				WithEvents([]string{"oss:ObjectCreated:PostObject"}).
				WithFilterKeyPrefix("r").
				WithFilterKeySuffix("s"),
		)
	...
	getTriggerOutput, err := client.GetTrigger(
		fc.NewGetTriggerInput(serviceName, functionName, triggerName),
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
	} else {
	...
	updateTriggerOutput, err := client.UpdateTrigger(
		fc.NewUpdateTriggerInput(serviceName, functionName, triggerName).
			WithInvocationRole("acs:ram:cn-hangzhou:123:role2"),
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
	} else {
	...
	deleteTriggerOutput, err := client.DeleteTrigger(
		fc.NewDeleteTriggerInput(serviceName, functionName, triggerName),
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
	} else {
	...
	listTriggersOutput, err := client.ListTriggers(
		fc.NewListTriggersInput(serviceName, functionName),
	)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
	} else {
